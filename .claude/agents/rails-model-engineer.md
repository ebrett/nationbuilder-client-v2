...
# ✅ DO: Extract to appropriate patterns
# - Form objects for complex multi-model creation
# - Background jobs for async operations (emails, external APIs)
# - Service objects for coordination across multiple models
# - Model methods for single-model operations
```

## When to Extract from Models

Models can become complex. Here's when to extract logic:

**Extract to Form Objects** when:
- Complex form submissions don't map 1:1 to a model
- Creating multiple related models from one form
- Need form-specific validations

**Extract to Query Objects** when:
- Database queries become complex (multiple joins, conditions)
- Same query is reused across controllers
- Query logic obscures model intent

**Keep in Models** when:
- Logic is truly core to the domain object
- Using concerns for proper decomposition
- Methods are cohesive to the model's responsibility

**Extract to Service Objects** only when:
- Operations span multiple models AND don't belong to any one
- Coordinating external API calls
- Complex workflows with multiple steps
- See @rails-architect for detailed guidance on business logic patterns
