...
**Business Logic Patterns** - Choose the right tool for the job:

**Form Objects/Command Objects** - For complex form submissions that don't map 1:1 to a model:
```ruby
...
class UserRegistrationForm
  ...
  attr_accessor :email, :password, :organization_name

  validates :email, :password, :organization_name, presence: true
  ...
  def save
    ...
    ActiveRecord::Base.transaction do
      ...
      @organization = Organization.create!(name: organization_name, owner: @user)
...
# Usage in controller:
@users = ActiveUsersQuery.new(current_organization.users).call
```

**Service Objects** - Use ONLY when you have operations that:
- Span multiple models and don't belong to any single one
- Require external API calls or third-party integrations
- Implement complex business workflows with multiple steps
- Need to be tested in isolation from Active Record
- Don't fit the CRUD paradigm (e.g., "GenerateMonthlyReport", "ProcessPayment")

⚠️ **Key Insight**: Service objects solve *coordination* problems, not just "my controller is fat" syndrome.
...
class PaymentProcessor
  ...
  private
    def process_payment
      # External API call to Stripe, etc.
      StripeClient.charge(@order.total, @payment_method)
    end
...
**Model Methods** - Don't overlook the simple solution:
```ruby
class User < ApplicationRecord
  # Complex logic can live in models when it's truly user-centric
  def activate_premium!
    transaction do
      update!(premium: true, premium_activated_at: Time.current)
      SubscriptionMailer.welcome(self).deliver_later
      Metrics.track("premium_activation", user_id: id)
    end
  end
end
```

⚠️ **Anti-Pattern**: If your service object just wraps a single model method call, move that logic to the model instead. "Fat models" aren't inherently bad when you use concerns and proper decomposition.
